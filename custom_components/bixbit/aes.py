"""Minimal pure-Python AES implementation for WhatsMiner token auth.

This avoids adding 'cryptography' as a runtime dependency in Home Assistant.
Supports AES-128-ECB (16-byte key) and AES-256-ECB (32-byte key).
"""

from __future__ import annotations

# AES S-Box
_SBOX = (
    ,
)

_INV_SBOX = (
    ,
)

_RCON = (0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36)


def _xtime(a: int) -> int:
    return ((a << 1) ^ 0x1B) & 0xFF if a & 0x80 else (a << 1) & 0xFF


def _mix_single(a: int, b: int) -> int:
    result = 0
    aa = a
    bb = b
    for _ in range(8):
        if bb & 1:
            result ^= aa
        hi = aa & 0x80
        aa = (aa << 1) & 0xFF
        if hi:
            aa ^= 0x1B
        bb >>= 1
    return result


def _key_expansion(key: bytes) -> list[list[int]]:
    nk = len(key) // 4  # 4 for AES-128, 8 for AES-256
    nr = {4: 10, 8: 14}[nk]
    nb = 4
    w: list[list[int]] = []
    for i in range(nk):
        w.append(list(key[4 * i : 4 * i + 4]))
    for i in range(nk, nb * (nr + 1)):
        temp = list(w[i - 1])
        if i % nk == 0:
            temp = temp[1:] + temp[:1]
            temp = [_SBOX[b] for b in temp]
            temp[0] ^= _RCON[i // nk - 1]
        elif nk == 8 and i % nk == 4:
            temp = [_SBOX[b] for b in temp]
        w.append([w[i - nk][j] ^ temp[j] for j in range(4)])
    return w


def _add_round_key(state: list[list[int]], rk: list[list[int]]) -> None:
    for c in range(4):
        for r in range(4):
            state[r][c] ^= rk[c][r]


def _sub_bytes(state: list[list[int]]) -> None:
    for r in range(4):
        for c in range(4):
            state[r][c] = _SBOX[state[r][c]]


def _inv_sub_bytes(state: list[list[int]]) -> None:
    for r in range(4):
        for c in range(4):
            state[r][c] = _INV_SBOX[state[r][c]]


def _shift_rows(state: list[list[int]]) -> None:
    state[1] = state[1][1:] + state[1][:1]
    state[2] = state[2][2:] + state[2][:2]
    state[3] = state[3][3:] + state[3][:3]


def _inv_shift_rows(state: list[list[int]]) -> None:
    state[1] = state[1][-1:] + state[1][:-1]
    state[2] = state[2][-2:] + state[2][:-2]
    state[3] = state[3][-3:] + state[3][:-3]


def _mix_columns(state: list[list[int]]) -> None:
    for c in range(4):
        a = [state[r][c] for r in range(4)]
        state[0][c] = _mix_single(2, a[0]) ^ _mix_single(3, a[1]) ^ a[2] ^ a[3]
        state[1][c] = a[0] ^ _mix_single(2, a[1]) ^ _mix_single(3, a[2]) ^ a[3]
        state[2][c] = a[0] ^ a[1] ^ _mix_single(2, a[2]) ^ _mix_single(3, a[3])
        state[3][c] = _mix_single(3, a[0]) ^ a[1] ^ a[2] ^ _mix_single(2, a[3])


def _inv_mix_columns(state: list[list[int]]) -> None:
    for c in range(4):
        a = [state[r][c] for r in range(4)]
        state[0][c] = (
            _mix_single(0x0E, a[0]) ^ _mix_single(0x0B, a[1])
            ^ _mix_single(0x0D, a[2]) ^ _mix_single(0x09, a[3])
        )
        state[1][c] = (
            _mix_single(0x09, a[0]) ^ _mix_single(0x0E, a[1])
            ^ _mix_single(0x0B, a[2]) ^ _mix_single(0x0D, a[3])
        )
        state[2][c] = (
            _mix_single(0x0D, a[0]) ^ _mix_single(0x09, a[1])
            ^ _mix_single(0x0E, a[2]) ^ _mix_single(0x0B, a[3])
        )
        state[3][c] = (
            _mix_single(0x0B, a[0]) ^ _mix_single(0x0D, a[1])
            ^ _mix_single(0x09, a[2]) ^ _mix_single(0x0E, a[3])
        )


def _encrypt_block(block: bytes, round_keys: list[list[int]], nr: int) -> bytes:
    state = [[0] * 4 for _ in range(4)]
    for r in range(4):
        for c in range(4):
            state[r][c] = block[c * 4 + r]
    _add_round_key(state, round_keys[0:4])
    for rnd in range(1, nr):
        _sub_bytes(state)
        _shift_rows(state)
        _mix_columns(state)
        _add_round_key(state, round_keys[rnd * 4 : rnd * 4 + 4])
    _sub_bytes(state)
    _shift_rows(state)
    _add_round_key(state, round_keys[nr * 4 : nr * 4 + 4])
    out = bytearray(16)
    for r in range(4):
        for c in range(4):
            out[c * 4 + r] = state[r][c]
    return bytes(out)


def _decrypt_block(block: bytes, round_keys: list[list[int]], nr: int) -> bytes:
    state = [[0] * 4 for _ in range(4)]
    for r in range(4):
        for c in range(4):
            state[r][c] = block[c * 4 + r]
    _add_round_key(state, round_keys[nr * 4 : nr * 4 + 4])
    for rnd in range(nr - 1, 0, -1):
        _inv_shift_rows(state)
        _inv_sub_bytes(state)
        _add_round_key(state, round_keys[rnd * 4 : rnd * 4 + 4])
        _inv_mix_columns(state)
    _inv_shift_rows(state)
    _inv_sub_bytes(state)
    _add_round_key(state, round_keys[0:4])
    out = bytearray(16)
    for r in range(4):
        for c in range(4):
            out[c * 4 + r] = state[r][c]
    return bytes(out)


def aes_ecb_encrypt(data: bytes, key: bytes) -> bytes:
    """Encrypt data with AES-ECB. Supports 16-byte (AES-128) or 32-byte (AES-256) keys."""
    assert len(key) in (16, 32)
    assert len(data) % 16 == 0
    nr = {16: 10, 32: 14}[len(key)]
    rk = _key_expansion(key)
    result = bytearray()
    for i in range(0, len(data), 16):
        result.extend(_encrypt_block(data[i : i + 16], rk, nr))
    return bytes(result)


def aes_ecb_decrypt(data: bytes, key: bytes) -> bytes:
    """Decrypt data with AES-ECB. Supports 16-byte (AES-128) or 32-byte (AES-256) keys."""
    assert len(key) in (16, 32)
    assert len(data) % 16 == 0
    nr = {16: 10, 32: 14}[len(key)]
    rk = _key_expansion(key)
    result = bytearray()
    for i in range(0, len(data), 16):
        result.extend(_decrypt_block(data[i : i + 16], rk, nr))
    return bytes(result)
